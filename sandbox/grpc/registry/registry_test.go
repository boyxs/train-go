...
// TestRegistryRoundTrip 是自包含的自动化测试:用 EtcdRegistry 做一次
...
func TestRegistryRoundTrip(t *testing.T) {
	...
	// 用 EtcdRegistry 注册:服务名 user-test(避免和真实 service/user 冲突),
	// 带上 Weight/Metadata 走一遍扩展字段写入路径。
	...
	defer regCancel()
	require.NoError(t, reg.Register(regCtx, ins))
	defer func() {
		require.NoError(t, reg.Close()) // 停续租 + 注销所有端点,不泄漏 goroutine
	}()
