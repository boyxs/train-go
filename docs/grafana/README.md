...
webook 项目 Grafana 完整指南。当前部署：`webook-grafana`（v11.6.0），端口 `3001`，配置 `deploy/grafana/provisioning/`。
...
### ⭐ 模板参考（编辑 `deploy/grafana/` 下文件时查这两份）
...
- **[`deploy/grafana/examples/`](../../deploy/grafana/examples/README.md)** — 每种文件类型一份 `-example.yml` / `.json`，和 `provisioning/` 平级单独目录（Grafana 不加载）。复制、删注释、填值即可用。
- **[`deploy/prometheus/examples/`](../../deploy/prometheus/examples/README.md)** — Prometheus `prometheus.yml` 主配置 + alert/recording rules 模板参考。
...
# 4. 推荐 dashboard 模板：见 deploy/grafana/provisioning/dashboards/README.md
```
