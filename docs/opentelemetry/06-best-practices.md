...
| 13 | **Gin `*gin.Context.Value()` 默认不 fallback** | HTTP span 和 DB/Redis 子 span 断成独立 trace | `server.ContextWithFallback = true` |
| 14 | **依赖启动竞态**（Kafka/ES 比应用慢） | 启动时 `connection refused`，运行期 noop 降级 | compose `depends_on: service_healthy` + 代码指数退避重试 |
| 15 | **OTel Collector kernel 兼容** | CentOS 7 kernel 3.10 报 `exec /otelcol: no such file` | 用 0.88.0 老版本，或升级 OS |
| 16 | **otelsarama 不支持 IBM/sarama** | 没有官方 instrumentation | 自写 header carrier + Producer/Consumer span 包装 |
