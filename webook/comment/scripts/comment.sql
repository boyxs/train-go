...
CREATE TABLE `comment` (
  ...
  `content`    varchar(1000) NOT NULL DEFAULT ''     COMMENT '评论内容（业务限 ≤500 字；占位删除后清空）',
  `reply_cnt`  bigint        NOT NULL DEFAULT 0      COMMENT '回复数：一级评论=整楼回复数（写/删回复增减楼根）；楼内回复恒 0',
  `deleted`    tinyint(1)    NOT NULL DEFAULT 0      COMMENT '占位删除标记：1=有子回复时保留的「该评论已删除」占位（内容清空，子树仍在）',
  ...
  -- 自关联外键：pid 引用本表 id。删除走软删（UPDATE deleted_at）不触发 CASCADE，子树保留；
  -- CASCADE 仅在物理 DELETE 时级联删整棵子树。
