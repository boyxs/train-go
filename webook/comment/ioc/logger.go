...
// InitLogger 与主仓 / chat 同源：prod/staging 用 production logger，其它用 development；
// yaml logger.level.l 覆盖等级；ginx 全局 logger 同步注入。
func InitLogger() logger.LoggerX {
	...
	l.Sugar().Infof("[comment] logger config: %+v", cfg)
