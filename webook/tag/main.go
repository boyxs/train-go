...
func main() {
	...
	// gRPC server：tag 的核心对外接口，供 core BFF 调用
	go func() {
		if err := app.GRPCServer.Register(); err != nil {
			log.Printf("[tag][gRPC] etcd 注册失败: %v", err)
			return
		}
		log.Printf("[tag][gRPC] serving on %s", app.GRPCServer.Addr)
		if err := app.GRPCServer.Serve(); err != nil {
			log.Printf("[tag][gRPC] server exited: %v", err)
		}
	}()

	// 最小 HTTP：仅 /metrics（Prometheus 抓取）+ /health（tag 不直接对前端，业务走 gRPC）
	...
	mux.HandleFunc("/health", func(w http.ResponseWriter, _ *http.Request) {
		...
		_, _ = w.Write([]byte(`{"status":"ok","service":"tag"}`))
	})
	httpSrv := &http.Server{Addr: httpAddr, Handler: mux}
	go func() {
		log.Printf("[tag][HTTP] metrics/health on %s", httpAddr)
		if err := httpSrv.ListenAndServe(); err != nil && !errors.Is(err, http.ErrServerClosed) {
			log.Fatalf("[tag][HTTP] exited: %v", err)
		}
	}()

	<-ctx.Done()
	log.Println("[tag][shutdown] 收到信号，开始优雅停机…")
	if err := app.GRPCServer.Close(); err != nil {
		log.Printf("[tag][gRPC] 关闭: %v", err)
	}
	sctx, scancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer scancel()
	if err := httpSrv.Shutdown(sctx); err != nil {
		log.Printf("[tag][HTTP] 关闭: %v", err)
	}
	cleanup()
	log.Println("[tag][shutdown] 完成")
}
