...
```
              ...
              │            │            │
        ┌─────▼─────┐ ┌───▼────┐ ┌────▼─────┐
        │ UserHandler│ │Article │ │ OAuth2   │
        │            │ │Author/ │ │ Handler  │
        │            │ │Reader  │ │          │
        └─────┬──────┘ └───┬────┘ └────┬─────┘
              │            │            │
        ┌─────▼──────┐ ┌──▼──────────▼──────┐
        │UserService  │ │ArticleAuthorService│
        │             │ │ArticleReaderService│
        └─────┬──────┘ └──┬──────────┬──────┘
              │            │          │
        ┌─────▼──────┐ ┌──▼────┐ ┌──▼────────┐
        │UserRepo     │ │Author │ │Reader     │
        │(Cache+DAO)  │ │Repo   │ │Repo       │
        └──┬────┬─────┘ └┬───┬─┘ └┬──────────┘
           │    │         │   │    │
        ┌──▼──┐┌▼───┐ ┌──▼─┐┌▼──┐┌▼──────────┐
        │MySQL││Redis│ │DAO ││Cache│  │ReaderDAO │
        │User ││User │ │Author│Article│  │(published│
        │     ││Cache│ │     ││Cache │  │_article) │
        └─────┘└─────┘ └────┘└─────┘  └──────────┘
```

**双库设计：** 制作库（`article` 表）存所有文章，线上库（`published_article` 表）只存已发布文章。两个 DAO 各自操作独立表，Service 层协调跨库操作（Publish/Withdraw/Delete）。

## 核心依赖

| 包 | 用途 |
|---|---|
| `gin` | HTTP 框架 |[CLAUDE.md](../CLAUDE.md)
...
| 文章(作者) | `ArticleAuthorHandler` | `web/article.go` | `InternalArticleAuthorHandler` |
| 文章(作者) | `ArticleAuthorService` | `service/article.go` | `InternalArticleAuthorService` |
| 文章(作者) | `ArticleAuthorRepository` | `repository/article_author.go` | `CacheArticleAuthorRepository` |
| 文章(作者) | `ArticleAuthorDAO` | `dao/article_author.go` | `GormArticleAuthorDAO` |
| 文章(作者) | `ArticleCache` | `cache/article.go` | `RedisArticleCache` |
| 文章(读者) | `ArticleReaderHandler` | `web/article.go` | `InternalArticleReaderHandler` |
| 文章(读者) | `ArticleReaderService` | `service/article.go` | `InternalArticleReaderService` |
| 文章(读者) | `ArticleReaderRepository` | `repository/article_reader.go` | `CacheArticleReaderRepository` |
| 文章(读者) | `ArticleReaderDAO` | `dao/article_reader.go` | `GormArticleReaderDAO` |
