...
func TestTimeoutFailoverClient_ChatStream(t *testing.T) {
	...
	testCases := []struct {
		name    string
		mock    func(ctrl *gomock.Controller) []llm.Client
		wantErr string
	}{
		{
			name: "主 provider 成功",
			mock: func(ctrl *gomock.Controller) []llm.Client {
				c0 := llmmocks.NewMockClient(ctrl)
				c0.EXPECT().ChatStream(gomock.Any(), gomock.Any(), gomock.Any()).
					Return(makeCh(), nil)
				return []llm.Client{c0}
			},
		},
		{
			name: "业务错误不计入故障",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			f := llm.NewTimeoutFailoverClient(tc.mock(ctrl), 3, logger.NewNopLogger())
...
func TestTimeoutFailoverClient_TimeoutTriggersSwitch(t *testing.T) {
	...
	c0 := llmmocks.NewMockClient(ctrl)
	c1 := llmmocks.NewMockClient(ctrl)
	...
	f := llm.NewTimeoutFailoverClient([]llm.Client{c0, c1}, 3, logger.NewNopLogger())
...
func TestTimeoutFailoverClient_Concurrent(t *testing.T) {
	...
	f := llm.NewTimeoutFailoverClient([]llm.Client{c0}, 3, logger.NewNopLogger())
