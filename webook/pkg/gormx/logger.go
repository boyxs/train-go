...
// GormLogger 实现 GORM logger.Interface：SQL 经 LoggerX.WithContext(ctx) 输出结构化字段
// （sql/rows/elapsed_ms + 请求内自带 trace.id）。正常 SQL 走 Debug（prod info 静默），
// 真正的 DB 错误（非 ErrRecordNotFound）升 Error，确保 prod 也能在 ELK 看到 SQL 失败。
...
func (g *GormLogger) Trace(ctx context.Context, begin time.Time, fc func() (sql string, rowsAffected int64), err error) {
	...
	// 真正的 DB 错误升 Error（ErrRecordNotFound 与正常 SQL 走 Debug）
