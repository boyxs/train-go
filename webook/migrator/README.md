...
[migrator] listening on :8083
```

服务监听 `:8083`。下面所有 curl 都打这个端口。
...
- 暴露宿主端口（`MIGRATOR_HOST_PORT=8083` 等），方便 IDE 调试
...
# webook-migrator   Up X minutes (healthy)   0.0.0.0:8083->8083/tcp

docker logs webook-migrator --tail 20 | grep "listening on"
# [migrator] listening on :8083
```

通过 nginx 反代访问（仅内网/白名单 IP；`/api/migrator/*` 剥前缀转 :8083）：

```bash
curl -sS http://localhost/api/migrator/tasks   # → 经 nginx → webook-migrator:8083/migrator/tasks
```
...
切回 go run 模式调试：先 `docker compose -p webook-local stop webook-migrator` 让容器停掉，再 `go run .` 占用 8083 端口。
...
curl -sS http://localhost:8083/metrics | head -5
...
```bash
curl -sS -X POST http://localhost:8083/migrator/tasks \
...
```bash
# 列表
curl -sS 'http://localhost:8083/migrator/tasks?offset=0&limit=10' | jq
# 期望：{"code":0,"data":{"list":[{TaskVO}],"total":1}}

# 详情
curl -sS http://localhost:8083/migrator/tasks/$TID | jq
...
```bash
curl -sS -X POST http://localhost:8083/migrator/preflight \
  -H 'Content-Type: application/json' \
...
```bash
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID/start \
  -H 'Content-Type: application/json' \
...
curl -sS http://localhost:8083/migrator/tasks/$TID | jq .data.status
# 期望：3 (IncrRunning)
```
...
```bash
curl -sS http://localhost:8083/migrator/tasks/$TID/lag | jq
...
```bash
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID/pause
...
```bash
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID/verify \
  -H 'Content-Type: application/json' \
  -d '{"mode":"full"}'
...
```bash
curl -sS "http://localhost:8083/migrator/tasks/$TID/mismatch?offset=0&limit=10" | jq
# 期望：list 3 行
...
# mark_only：只标记不动数据
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID/repair \
  -H 'Content-Type: application/json' \
...
```bash
# 设 50%
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID/gray \
  -H 'Content-Type: application/json' \
  -d '{"percent":50}'
...
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID/switch \
  -H 'Content-Type: application/json' \
  -d '{"stage":"SRC_FIRST"}'
...
```bash
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID/replay-dl \
  -H 'Content-Type: application/json' \
  -d '{"limit":1000}'
...
```bash
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID2/start \
  -H 'Content-Type: application/json' -d '{"phase":"full"}'
sleep 2
...
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID2/pause
```
...
```bash
# 全量对账（干净迁移）—— 零假阳性
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID2/verify \
  -H 'Content-Type: application/json' -d '{"mode":"full"}'
...
# 查差异明细（与同构 A8.3 同形）
curl -sS "http://localhost:8083/migrator/tasks/$TID2/mismatch?offset=0&limit=10" | jq
...
# 看一下任务详情
curl -sS http://localhost:8083/migrator/tasks/$TID3 | jq
...
```bash
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID3/start \
  -H 'Content-Type: application/json' \
  -d '{"phase":"full"}'
...
```bash
curl -sS "http://localhost:8083/migrator/tasks/$TID3/mismatch?offset=0&limit=10" | jq
# 期望：list 3 行
#   bizId="2"  mismatchKind="missing"
#   bizId="3"  mismatchKind="diff"  diffDetail 含 "diff_fields":["title"]
#   bizId="99" mismatchKind="extra"
...
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID3/repair \
  -H 'Content-Type: application/json' \
...
```bash
# 推 SRC_FIRST
curl -sS -X POST http://localhost:8083/migrator/tasks/$TID3/switch \
...
| curl 返 ConnRefused | migrator 没起来或端口被占（默认 :8083，看启动日志） |
