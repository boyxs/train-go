...
## [2026-07-24] Feed 计数修复 + BFF 聚合并发 + 可观测性/消费者收敛

**变更内容**: 承接 [2026-07-23] Feed P0 的联调修复 + 基础设施收敛。① **新内容计数改「读时过滤」**：原 `NewCount` 对收件箱裸 `ZCOUNT`，而撤回文章按设计「读时过滤、inbox 不摘除」仍赖在收件箱 → 撤回文章被当新文章计数（删文章冒新数 / 点列表后反复提示）。改为 feed `NewCount` 返回 since 以来候选 `articleId`（`InboxSince` ZREVRANGEBYSCORE，capped `new_count_max`），core BFF 用 `CountByIds` 过滤可见（撤回=软删天然滤掉）后计数，与列表同口径（proto `NewCountResponse.count`→`article_ids`）。② **新增轻量 `CountByIds`**（dao `SELECT COUNT(*) … WHERE id IN` + `.Model` 软删过滤 / repo 按 `ids[0]` 迁移路由 / service），替代为「数个数」而 `BatchDetail` 捞全字段。③ **BFF `List` 五源聚合串行→并发**（`sync.WaitGroup`，4 个独立降级源并发，延迟从四者之和→最大值）。④ **client 出站日志拦截器统一**：core/chat/worker 的下游 dial 补 `logging.BuildUnaryClient`（原只 feed 有），失败记 `grpc.method`+`grpc.code`+`trace.id` 定位「哪个下游挂了」。⑤ **消费者调度骨架下沉 `pkg/saramax`**：`runConsumerGroup`→`saramax.RunGroup`+`GroupConfig`（连接自管+退避重连，升公共 API 补零值兜底防紧忙重连），worker/consumer 各消费者复用；移除 `ConsumerConfig` 别名、直接用 `saramax.GroupConfig`。⑥ **消费者组命名规范化**：base `interaction-consumer`→`webook-worker`（5 yaml，env 全不带），三消费者对称派生 `webook-worker-{interaction,feed-article,feed-relation}`。⑦ **`new_count_max` 提进 feed config**（5 yaml + FeedConfig，默认 99）。
**影响范围**: `api/proto/feed`（NewCountResponse + gen，protoc-gen-go 对齐仓库 v1.36.4，兄弟 pb 零 churn）；`webook/feed/`（service/repository/cache `InboxSince`、grpc、ioc/feedcfg、5 yaml `new_count_max`）；`webook/internal/`（service/feed BFF `NewCount`+`List`、service/article + repository{,/dao}/article_reader `CountByIds`、ioc/grpc client 日志、3 mock 重生成）；`webook/{chat,worker}/ioc/grpc.go` client 日志；`pkg/saramax/group.go`（新）；`webook/worker/`（consumer 骨架下沉 + 5 yaml `consumer_group` + ioc/kafka + wire）；`deploy/.env.prod.example`（CORE/FEED/WORKER `IMAGE_TAG` bump）；`webook/feed/CLAUDE.md`。
**技术决策**: ① **计数与列表同口径**——feed 无 MySQL/不知可见性，候选 id 交 BFF 用现成软删过滤判定，契合「撤回=读时过滤」既定设计，且不依赖删除事件（丢事件也不误报）；② **计数用 `COUNT(*)` 不用 `BatchDetail`**——只需数量不需字段，`len(ids)==0` 短路不打库；③ **client 日志全服务铺开**而非只 feed——「哪个服务报错」属可观测性职责（错误对象不带服务名以免泄漏拓扑），靠 `grpc.method`+`trace.id`；④ **saramax 用 struct + 别名复用而非并行同构类型**（对齐既有偏好）；⑤ **组名 service 域 base + 对称派生**——原 base 用单消费者名（interaction）当前缀不合理，env 不进组名（各环境独立集群）；⑥ 修正 [2026-07-23] 记录「抽 `runConsumerGroup` 共享骨架」——已进一步下沉到 `pkg/saramax.RunGroup`。
**待办**: ① **组名变更 = Kafka 新组、无历史 offset → 首读 OffsetOldest 重放**（interaction 读数重复累加一次、feed 幂等无碍），部署时知悉；② fanout 事件 best-effort，丢事件那篇文章要等收件箱 built TTL（7d）或关注变更重建才补回，无对账兜底（可加发布时间兜底扫描 / 事务性 outbox）；③ 残留已撤回文章仍占收件箱、每次轮询打一次轻量 COUNT，直到重建清理（读时过滤既定取舍）；④ `ArticleReaderService.AuthorArticles` 疑似同款串行聚合，未并发化（另开）；⑤ `IMAGE_TAG` bump 为提议补丁位，实际发版需推 `webook-{core,feed,worker}-v*` tag 并同步真实 `.env.prod`。
**会话**: 260724-feed-计数修复与可观测性收敛
**发布**: 待上线（core/feed/worker 出镜像 + 推对应 tag 生效）

## [2026-07-23] Feed 关注流：webook-feed 独立服务 + core BFF + worker 消费 + 前端双 Tab（P0 全量落地）

**变更内容**: 按 `prd/feed/`（PRD + ARCHITECTURE）走 workflow:tdd 全量实现关注流 P0（22 任务 6 阶段）。**新增独立 gRPC 服务 `webook/feed/`**（8100/8101，**无 MySQL**，收件箱/发件箱全 Redis 投影）：cache 层（inbox/outbox/bigv/built + 「存在才追加」Lua 原子 + jitter TTL + pipeline）→ repository（薄委托无 DAO）→ service（写 `Fanout`/`Remove`/`InvalidateInboxes` + 读 `ListFeed`+rebuild+归并，持 relation/article gRPC client）→ grpc 薄壳（pb↔domain + validate）；推拉结合（大 V 粉丝数 ≥ 阈值不扩散、读时归并 outbox）、关系变更=失效重建、撤回=读时过滤。**core**：新增 `internal/events/article`（生产者，key=authorId）+ `ArticleAuthorService.Publish/Withdraw` best-effort 产事件 + article gRPC `ListAuthorArticles`（DAO `ListBriefByAuthor` Select 排除 Content）+ BFF `service.GRPCFeedService` 五源聚合（feed+article+user+interaction+comment+tag，撤回过滤 + 单源失败降级）+ `web.InternalFeedHandler`（`POST /feed/list`，JWT）。**worker**：`consumer/event/{article,relation}.go` 契约副本 + contract_test 扩展 + `FeedArticleConsumer`/`FeedRelationConsumer`（批量、独立 group、退避重连，抽 `runConsumerGroup` 共享骨架）。**前端**：`api/feed.ts` + `types/feed.ts` + `hooks/useInfiniteScroll.ts`（IntersectionObserver 哨兵，复用 `useCursorList` 取数）+ 广场页双 Tab 壳（`views/feed/`，Tab 写 URL，发现 Tab 行为原样保留）+ 关注流 5 态（未登录引导/首屏/错误重试/空态/内容流，卡片含计数+标签 chip）。**契约**：`api/proto/feed/v1/feed.proto` 新增 + `article.proto` 加 `ListAuthorArticles`/`FeedArticleBrief`（raw protoc `make gen`）。
**影响范围**: 新增 `webook/feed/**`（全服务）；`api/proto/feed` + `api/proto/article`（+gen）；`internal/`（events/article、service/{article,feed}、web/feed、grpc/article_reader_server、repository+dao/article_reader、domain/article、ioc/{grpc,web}、wire、5 份 config client 段、events/article/mocks + 3 article mock 重生成）；`worker/`（consumer/{run,feed_article,feed_relation}+event 副本、ioc/grpc、wire、main、5 份 config）；`webook-fe/`（api/feed、types、hooks/useInfiniteScroll、views/feed、views/article/feed 抽壳、app/(auth)/feed/page）；`deploy/`（compose + local override + prometheus job + grafana webook-feed.yml + 4 份 .env.example FEED_*）；`.github/workflows/webook-feed-ci.yml` + 9 兄弟 CI paths-ignore；`mk/mock.mk`；`webook/feed/CLAUDE.md`。
**技术决策**: ① **feed 无 MySQL**——数据是缓存投影，源头在 core/relation，直查别库破边界，全 Redis + 丢失可重建；② **推拉结合**挡大 V 写放大（阈值 yaml 可调）；③ **失效重建**一招覆盖 follow/unfollow/block（含拉黑级联），代价首读慢；④ **outbox 存在才追加需 Lua 原子**防假全量缓存；⑤ **消费归 worker**（worker 铁律收拢异步），feed 保持纯同步 gRPC；⑥ **article 事件 key=authorId** 保同作者 publish→withdraw 有序；⑦ **BFF 五源聚合在 service 层**（撤回过滤靠 `BatchDetail` 查线上库，interaction/comment/tag/user 失败降级填零不阻断）；⑧ **fanout_batch 默认 50**（relation `ListFollowers` limit 服务端封顶 50，设计原写 500）；⑨ **前端复用 `useCursorList` 取数 + 新 `useInfiniteScroll` 只管哨兵**（职责分离，≥2 列表可复用）；⑩ **Metric 统一 `webook_grpc_*`**（复用共享 builder，job 标签区分 service，不加 `webook_feed_*`）。
**待办**: ① P1 `NewCount` + `/feed/new-count` + 提示条**已实现**（30s 轮询）；② Kafka 新 topic `article_events` 建法对齐部署（broker 自动建 vs 显式建）需运行时确认；③ core/worker/feed 单测 + feed 集成（真 Redis）已绿，**跨服务端到端（真起 feed+core+worker+kafka）联调**待部署环境验证；④ `deploy/.env.<env>` 实际值由部署者按 example 同步 `FEED_IMAGE_TAG`/`FEED_APP_ENV`。**ship review 已修**（自动重试风暴门控 `!error`、feed 层 limit 夹取、`.env.dev/staging/local` 补 `FEED_IMAGE_TAG`、`Delete` 补撤回事件、hex→PALETTE token 等）；**遗留已记录取舍**：⑤ 游标为标量 `publishedAt`，同毫秒边界条目跨页可能跳/重（ARCHITECTURE §3 已接受；如要精确→升复合游标 `publishedAt+articleId`，属契约变更）；⑥ 零文章大 V 的 outbox 首页读每次回源（无负缓存，边界穿透，自愈于 TTL）；⑦ 多 key Redis 操作（Invalidate/跨 uid pipeline）未做 Cluster hash-tag，当前单机 Redis OK，上 Cluster 需改。
**会话**: 260723-feed-关注流P0实现
**发布**: 待上线（新服务 + core/worker 改动 + 前端，随下次出镜像 + 推 `webook-feed-v*` tag 生效）
