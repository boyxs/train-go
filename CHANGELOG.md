...
**待办**: ① **ELK 运行时验证**（`COMPOSE_PROFILES=elk ./deploy.sh dev` 起栈 → `ES_HOST=... ./elk/es/setup.sh` 建 ILM+模板 → Kibana 建 Data View `webook-logs-*` → 造一次跨服务请求验 trace.id 关联 + 验 `@timestamp` 为日志真实时间而非入库时间——本次 ship review 已修 logstash 时间戳撞车，但 webook-es 处 paused 未能实跑，随本项一并验）；② Kibana `trace.id` Url formatter 跳 Zipkin + Grafana 日志告警规则（PRD §12）；③ ES 角色隔离硬化（现 Logstash/Kibana 用 elastic 超管，prod 建 logstash_writer/kibana_reader）；④ Logstash Prometheus 指标需 logstash-exporter（当前仅 docker healthcheck）；⑤ gRPC client 侧 logging 拦截器（本期只接 server 侧）可选补。
